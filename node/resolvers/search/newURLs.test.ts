...
import { VBase, IOContext, VBaseSaveResponse } from '@vtex/api'
...
describe('Search new URLs dicovery', () => {
  ...
  class VBaseMock extends vbaseTypeMock.object {
    ...
    public saveJSON = async <T>(
      ...
      /**
       * Fixed saved empty return to at least return something
       * (instead of void, which doesn't match the function signature)
       */
      return []
    }
  }
  ...
  it('Should transform /category in /category?map=c', async () => {
    ...
    /**
     * Fixed to have all previous properties from facetMock, and then
     * being able to overwrite SpecificationFilters
     */
    const facets = {
      ...facetsMock.object,
  ...
  it('Should transform /department/style_1/color_2/size_3 in /department/1/2/3?map=c,specificationFilter_1,specificationFilter_2,specificationFilter_3', async () => {
     ...
     * Fixed to have all previous properties from facetMock, and then
     * being able to overwrite SpecificationFilters.
     * Fixed to have missing required attributes too (Quantity, Link, LinkEncoded)
     */
    const facets = {
      ...facetsMock.object,
      SpecificationFilters: {
    ...
    expect(result).toStrictEqual({
      ...
      map:
        'c,specificationFilter_1,specificationFilter_2,specificationFilter_3',
    })
  })
